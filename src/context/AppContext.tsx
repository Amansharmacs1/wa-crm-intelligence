...
export type AppRoute = 'landing' | 'dashboard' | 'all-leads' | 'hot-leads' | 'at-risk-leads' | 'follow-ups' | 'agents' | 'lead-details' | 'settings' | 'login' | 'register' | 'signup' | 'auth-callback';
...
interface AppContextType {
  ...
  loginWithQR: () => void;
  loginWithMagicLink: (userData: { id: string; email: string; name: string }) => Promise<void>;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginWithMagicLink = async (userData: { id: string; email: string; name: string }) => {
    ...
    setUser(magicUser);
    localStorage.setItem('wacrm_user', JSON.stringify(magicUser));
  };
